...
function convertTrialToCustomer($trialId) {
    ...
    if ($conn) {
        ...
        $trials = isset($_SESSION['trials']) ? $_SESSION['trials'] : [];
        $trial = null;
        foreach ($trials as $t) {
            if ($t['subid'] == $trialId) {
                $trial = $t;
    ...
    if (!isset($jsonResponseAuth["access_token"])) {
        error_log("convertTrialToCustomer: Token exchange failed - access_token not found");
        return "token_failed";
    }
    ...
    if(strpos($response, "already exist") !== false) {
        error_log("convertTrialToCustomer: Account already exists for email: " . $email);
        return "already_exist";
    } elseif(strpos($response, 'externalId') !== false) {
        error_log("convertTrialToCustomer: Subscriber created successfully for email: " . $email);
    } else {
        error_log("convertTrialToCustomer: Subscriber creation failed. Response: " . substr($response, 0, 500));
